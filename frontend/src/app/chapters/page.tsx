...
import { CHAPTER_ICON_OPTIONS, getChapterIcon } from '@/constants/chapter-icons';
...
export default function ChaptersPage() {
    ...
    const [formData, setFormData] = useState({ name: '', description: '', color: '#6366f1', icon: 'book-open' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFormData({ name: '', description: '', color: '#6366f1', icon: 'book-open' });
            }
        } catch (error) {
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <Link href="/dashboard" className="p-2 rounded-lg text-neutral-400 hover:text-white hover:bg-white/10 transition-all">
                        ...
                        <div>
                            ...
                            <p className="text-neutral-400">Chapters of your life, archived as digital volumes</p>
                        </div>
                    </div>
                    <button
                        onClick={() => { setShowModal(true); setEditingChapter(null); setFormData({ name: '', description: '', color: '#27272a', icon: 'book-open' }); }}
                ...
                {isLoading ? (
                    ...
                    <div className="glass-card p-12 rounded-2xl text-center">
                        ...
                        <p className="text-neutral-400 mb-6">Create chapters to organize your journal entries into meaningful collections.</p>
                        <button
                                ...
                                <div className="h-32 relative overflow-hidden bg-neutral-900">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        {(() => {
                                            ...
                                            return <ChapterIcon className="w-12 h-12 text-white group-hover:scale-125 transition-transform duration-700" />;
                                        })()}
                                    </div>
                                    <div className="absolute bottom-0 left-0 right-0 h-1" style={{ backgroundColor: chapter.color }} />
                                </div>
                                <div className="p-6 relative">
                                    <div className="absolute -top-4 right-6 px-3 py-1 rounded-full bg-neutral-900 border border-white/10 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => openEditModal(chapter)} className="p-2 rounded-lg text-neutral-400 hover:text-white hover:bg-white/10 transition-all">
                                    ...
                                    {chapter.description && <p className="text-neutral-400 text-sm mb-6 line-clamp-2 italic">"{chapter.description}"</p>}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary/50"
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-neutral-400 mb-2">Icon</label>
                                <div className="flex flex-wrap gap-2">
                                    {CHAPTER_ICON_OPTIONS.map(({ value, Icon }) => (
                                        <button
                                            key={value}
                                            type="button"
                                            onClick={() => setFormData({ ...formData, icon: value })}
                                            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${getChapterIcon(formData.icon) === Icon ? 'bg-primary' : 'bg-white/5 hover:bg-white/10'}`}
                                        >
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {COLORS.map((color) => (
                                        <button
                                            ...
                                            className={`w-8 h-8 rounded-full transition-all ${formData.color === color ? 'ring-2 ring-white ring-offset-2 ring-offset-neutral-900' : ''}`}
