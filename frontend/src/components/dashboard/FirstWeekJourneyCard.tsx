...
export default function FirstWeekJourneyCard({ steps }: { steps: JourneyStep[] }) {
    ...
    return (
        <section className="rounded-card-125 border border-[rgba(216,199,232,0.28)] bg-[rgba(216,199,232,0.1)] p-3.5">
            <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full border border-[rgba(92,92,92,0.1)] bg-[rgba(255,255,255,0.42)] px-2 py-1 text-[0.62rem] font-semibold text-[rgb(var(--text-soft))]">
                    {completeCount}/{steps.length}
                </span>
            </div>
            <div className="mt-3 space-y-2">
                {steps.map((step) => (
                    <div key={step.label} className="flex items-start gap-2.5 rounded-card-95 border border-[rgba(92,92,92,0.1)] bg-[rgba(255,255,255,0.36)] px-3 py-2.5">
                        ...
                        <div>
                            ...
                            <p className="mt-0.5 text-[0.68rem] leading-5 text-[rgb(var(--text-soft))]">{step.hint}</p>
                        </div>
                    </div>
                ))}
            </div>
