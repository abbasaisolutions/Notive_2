...
/** Generate a clean HTML interview prep sheet from STAR stories. */
const buildInterviewPrepHtml = (stories: Overview['interviewStories']): string => {
    const rows = stories.map((s, i) => `
        <section class="story">
            <h2>${i + 1}. ${s.title || 'Untitled story'}</h2>
            ${s.situation ? `<div class="field"><span class="label">Situation</span><p>${s.situation}</p></div>` : ''}
            ${s.task ? `<div class="field"><span class="label">Task</span><p>${s.task}</p></div>` : ''}
            ${s.action ? `<div class="field"><span class="label">Action</span><p>${s.action}</p></div>` : ''}
            ${s.result ? `<div class="field"><span class="label">Result</span><p>${s.result}</p></div>` : ''}
        </section>`).join('');
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Interview Prep Sheet</title><style>
        body{font-family:Georgia,serif;max-width:740px;margin:40px auto;padding:0 24px;color:#1a1a1a;line-height:1.7}
        h1{font-size:1.5rem;margin-bottom:4px}.meta{color:#666;font-size:.85rem;margin-bottom:2rem}
        .story{page-break-inside:avoid;margin-bottom:2.5rem;padding-bottom:2rem;border-bottom:1px solid #e0dbd4}
        .story:last-child{border-bottom:none} h2{font-size:1.1rem;margin-bottom:1rem;color:#333}
        .field{margin-bottom:.85rem} .label{display:block;font-size:.7rem;text-transform:uppercase;letter-spacing:.1em;color:#888;margin-bottom:2px}
        p{margin:0;font-size:.95rem}
        @media print{body{margin:20px}}
    </style></head><body>
        <h1>Interview Prep Sheet</h1>
        <p class="meta">${stories.length} STAR stor${stories.length === 1 ? 'y' : 'ies'} · Generated by Notive · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
        ${rows}
    </body></html>`;
};
...
const buildGrowthReportHtml = (overview: Overview, trends: Trends | null): string => {
    const skills = overview.topSkills.slice(0, 8).map((skill) => `<li>${escapeReportHtml(skill)}</li>`).join('') || '<li>Keep capturing moments to surface strengths.</li>';
    const lessons = overview.topLessons.slice(0, 8).map((lesson) => `<li>${escapeReportHtml(lesson)}</li>`).join('') || '<li>Lessons will appear as your evidence base grows.</li>';
    const trendRows = trends?.points.map((point) => `
        <tr><td>${escapeReportHtml(point.periodLabel)}</td><td>${point.entries}</td><td>${point.verified}</td><td>${Math.round(point.averageConfidence * 100)}%</td></tr>`).join('') || '';
    const profile = overview.profileContext;

    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Notive Growth Report</title><style>
        body{font-family:Georgia,serif;max-width:760px;margin:40px auto;padding:0 24px;color:#242424;line-height:1.6}
        h1{font-size:1.65rem;margin:0 0 4px}h2{font-size:1rem;margin:28px 0 8px;text-transform:uppercase;letter-spacing:.1em;color:#59684a}
        .meta{margin:0;color:#666;font-size:.85rem}.summary{margin-top:26px;padding:18px;border:1px solid #d9ded0;background:#f8faf4}
        .stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:18px}.stat{padding:12px;border:1px solid #e5e1da}.stat strong{display:block;font-size:1.25rem}
        ul{margin:0;padding-left:20px}li{margin:5px 0}table{width:100%;border-collapse:collapse;margin-top:10px;font-family:Arial,sans-serif;font-size:.85rem}
        th,td{padding:9px;border-bottom:1px solid #e5e1da;text-align:left}th{color:#59684a;text-transform:uppercase;font-size:.7rem;letter-spacing:.08em}
        @media print{body{margin:20px}.summary{break-inside:avoid}}
    </style></head><body>
        <h1>Growth Report</h1>
        <p class="meta">Generated by Notive · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
        <section class="summary"><strong>${escapeReportHtml(profile?.primaryGoal || 'Your growth snapshot')}</strong><br>${escapeReportHtml(profile?.focusArea || 'A record of the moments, strengths, and lessons you have captured.')}</section>
        <div class="stats"><div class="stat"><strong>${overview.stats.entryCount}</strong>memories</div><div class="stat"><strong>${overview.stats.experienceCount}</strong>story seeds</div><div class="stat"><strong>${overview.stats.verifiedCount}</strong>verified stories</div></div>
        <h2>Strengths surfacing</h2><ul>${skills}</ul>
        <h2>Lessons carrying forward</h2><ul>${lessons}</ul>
        ${trendRows ? `<h2>Recent momentum</h2><table><thead><tr><th>Period</th><th>Entries</th><th>Verified</th><th>Confidence</th></tr></thead><tbody>${trendRows}</tbody></table>` : ''}
    </body></html>`;
};
...
export default function PortfolioWorkspace() {
    ...
    const downloadGrowthReport = useCallback(() => {
        if (!overview) return;
        setDownloadingKey('growth:html');
        try {
            triggerDownload(
                `notive-growth-report-${new Date().toISOString().slice(0, 10)}.html`,
                buildGrowthReportHtml(overview, trends),
    ...
    const printGrowthReport = useCallback(() => {
        if (!overview) return;
        const printWindow = window.open('', '_blank');
        if (!printWindow) return;

        printWindow.opener = null;
        setDownloadingKey('growth:print');
        printWindow.document.open();
        printWindow.document.write(buildGrowthReportHtml(overview, trends));
        printWindow.document.close();
        printWindow.focus();
        window.setTimeout(() => {
            printWindow.print();
                        ...
                        <div id="portfolio-interview-tools" className="space-y-4">
                            ...
                            <div className="workspace-panel rounded-card-163 p-4">
                                ...
                                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                                    <button
                                        type="button"
                                        disabled={downloadingKey === 'interview:html'}
                                        onClick={() => {
                                            setDownloadingKey('interview:html');
                                            try {
                                                triggerDownload(
                                                    `notive-interview-prep-${new Date().toISOString().slice(0,10)}.html`,
                                                    buildInterviewPrepHtml(stories),
                                                    'text/html; charset=utf-8',
                                                );
                                            } finally {
                                                setDownloadingKey(null);
                                        ...
                                        className="workspace-button-outline inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold disabled:opacity-60"
                                    >
                                        <FiDownload size={14} aria-hidden="true" />
                                        {downloadingKey === 'interview:html' ? 'Preparing...' : 'Download'}
                                    </button>
                                    <button
                                        type="button"
                                        disabled={downloadingKey === 'interview:print'}
                                        onClick={() => {
                                            const win = window.open('', '_blank');
                                            if (!win) return;
                                            win.opener = null;
                                            setDownloadingKey('interview:print');
                                            win.document.open();
                                            win.document.write(buildInterviewPrepHtml(stories));
                                            win.document.close();
                                            win.focus();
                                            window.setTimeout(() => { win.print(); setDownloadingKey(null); }, 200);
                                        }}
                                        className="workspace-button-outline inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold disabled:opacity-60"
                                    >
                                        <FiPrinter size={14} aria-hidden="true" />
                                        {downloadingKey === 'interview:print' ? 'Preparing...' : 'Print / PDF'}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {overview && (
                        <div className="flex flex-wrap gap-2">
                            <button
                                ...
                                disabled={downloadingKey === 'growth:html'}
                                className="workspace-button-outline inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold disabled:opacity-60"
                            >
                                <FiDownload size={14} aria-hidden="true" />
                                {downloadingKey === 'growth:html' ? 'Preparing...' : 'Download report'}
                            </button>
                            <button
                                type="button"
                                onClick={printGrowthReport}
                                disabled={downloadingKey === 'growth:print'}
                                className="workspace-button-outline inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold disabled:opacity-60"
                            >
                                <FiPrinter size={14} aria-hidden="true" />
                                {downloadingKey === 'growth:print' ? 'Preparing...' : 'Print / PDF'}
                            </button>
                        </div>
                    )}
