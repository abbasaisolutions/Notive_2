...
const formatMemoryCount = (count: number) => `${count} ${count === 1 ? 'memory' : 'memories'}`;
...
export default function TimelineView({
    ...
    return (
        <div className="relative py-3 md:py-10">
            <div className="absolute left-5 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-primary/50 via-primary/18 to-transparent" />
                        ...
                        >
                            ...
                            >
                                <div className="inline-flex flex-wrap items-center gap-2 rounded-full border border-[rgba(var(--paper-border),0.58)] bg-[rgba(var(--surface-2),0.82)] px-3.5 py-2 shadow-lg shadow-black/12 backdrop-blur-xl">
                                    ...
                                    <span className="text-[0.62rem] font-semibold uppercase tracking-[0.12em] text-ink-muted">
                                        &middot;
                                    </span>
                                    <span className="text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-ink-secondary">
                                        {formatMemoryCount(group.count)}
                                    </span>
                                    {season && (
                                        <span className="hidden md:inline-flex rounded-full border border-primary/30 bg-primary/12 px-2.5 py-0.5 text-[0.66rem] font-semibold uppercase tracking-[0.12em] text-primary">
                                            {season.title}
                                        </span>
                                    )}
                                </div>
